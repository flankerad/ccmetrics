...
Working on ccmetrics itself? `uv tool install .` copies the code, so edits in your clone never reach the `ccmetrics` your status line runs. Install it linked instead — `uv tool install --force --editable .` — and every save shows up on the next status-line render, with no restart of Claude Code.
...
Your status line becomes `ccmetrics > main | ✳ Opus 5 :: default 620k/1M (62%) | $21.25 | 5h 24% left | ████▌░░░ week 26% left` — repo, branch, model, output style, tokens used against the context window, what this session has cost so far, then how much of your 5-hour and weekly plan limits remains, with an eight-cell bar sitting beside the weekly figure. Every segment is optional: when a number is missing the segment disappears and the separators close up behind it. The bar fills left to right in eighths of a cell, so it moves at every percent or two rather than sitting still, and it colours each cell by its own position — the leftmost cells stay green and only a nearly-spent week shows red. It tracks the WEEKLY percentage specifically (the same number the text beside it prints), not context: a 200k-token context window dwarfs one conversation, so a context-driven bar would barely ever move, while the week number actually climbs over days. No subscription, so no weekly reading? No bar prints at all — context stays a plain number either way. The plan segments print what is left but stay coloured by what is spent, so a week with 5% left reads red, not green. The dollar figure stays plain because it has no ceiling to grade against. Meanwhile `ccmetrics dash` grows a **PLAN** card and `ccmetrics` prints a PLAN line. Only the percentage, the reset time and the session id are stored (90 days, metadata as always); readings older than 6 hours are labelled stale rather than shown as current. Undo the wiring and the card and the line simply stop appearing.

Three things worth knowing plainly about where that percentage comes from: it arrives only when Claude Code actually renders your status line — an idle session writes nothing, so the dash and widget just age until the next render. ccmetrics only ever reads what the terminal Claude Code writes on THIS machine; work done in claude.ai on the web never reaches it. And the percentage itself is account-wide, computed by Anthropic — the local files here only carry a copy of it, never derive or estimate it themselves.
